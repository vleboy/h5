module game {
	export class TopBarMediator extends BaseMediator implements INotification{
		public constructor() {
			super();
		}
		...
		/**初始化UI */
		protected initUI(){
			this.createUIByName(Mediators.Mediator_TopBar.uiName, Mediators.Mediator_TopBar.layer);
		}
		...
		handleNotification(type: number, body?: any): void{
			switch(type){
				case NotifyConst.topState:
					this.notifyUI(UICommand.TopBar_topState, body);
					break;
			}
		}
