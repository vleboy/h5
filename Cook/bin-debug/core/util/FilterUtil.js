...
(function (game) {
    var FilterUtil = (function () {
        ...
        /**流光 */
        FilterUtil.setLightFlowFilter = function (display, open) {
            if (open === void 0) { open = true; }
            if (open) {
                var filter_1 = new egret.CustomFilter(this.vertexSrc, this.fragmentSrc_lightFlow, {
                    customUniform: 0
                });
                display.filters = [filter_1];
                var frameCall = function () {
                    filter_1.uniforms.customUniform += 0.15;
                    if (filter_1.uniforms.customUniform > Math.PI * 2) {
                        filter_1.uniforms.customUniform = 0.0;
                    }
                };
                display.addEventListener(egret.Event.ENTER_FRAME, frameCall, this);
            }
